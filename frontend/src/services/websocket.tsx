...
class WebSocketClient {
   ...
   */
  public addMessageListener(type: string, listener: (message: WebSocketMessage) => void): void {
   ...
   */
  public removeMessageListener(type: string, listener: (message: WebSocketMessage) => void): void {
      ...
      if (typeof subscription === 'string') {
        if (subscription.startsWith('market:')) {
          ...
          if (parts.length > 1) {
            ...
            this.subscribeToMarket(currencyPair);
          }
  ...
  private notifyConnectionStateListeners(connected: boolean): void {
    if (!this.connectionStateListeners) return;
    
    // Safely iterate through listeners
    Array.from(this.connectionStateListeners).forEach(listener => {
...
// Create WebSocket client singleton - ensure it's only created on the client side
